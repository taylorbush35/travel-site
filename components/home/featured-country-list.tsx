    ...
    <ul className="mt-2 flex flex-col gap-3 md:gap-3.5">
      {countries.map((country) => (
        <li key={country.slug}>
          ...
          >
            <div className="grid grid-cols-1 items-start gap-5 md:grid-cols-12 md:items-center md:gap-x-10 md:gap-y-0 lg:gap-x-14">
              <div className="min-w-0 md:col-span-5">
                <p className="text-[10px] font-medium uppercase tracking-[0.2em] text-[var(--color-signature)]">
                  {country.region}
                </p>
                <h3 className="mt-2 text-[1.75rem] font-medium leading-[1.15] tracking-tight text-[var(--color-text-primary)] transition-colors group-hover:text-[var(--color-signature)] md:text-[2rem]">
                  {country.name}
                </h3>
              </div>
